...
$(document).ready(function() {
	...
	function initMenu(){
		firstMenuHide();
		$(".menu_bar").bind("click",function(e){
			e.stopPropagation();
			e.preventDefault();
			showMenu();
		});

		$(".gnbdim").bind("click",function(e){
			e.stopPropagation();
			e.preventDefault();
			hideMenu();
		});
	...
	function firstMenuHide(){
		...
		$(".allmenuIn").css({'transition' : 'all 0s ease-in-out','transform' : 'translateX(-100%)'});
	...
	function hideMenu(){
		isMenuOpen = false;
		$('.gnbdim').css("display","none");
		$(".allmenuIn").css({'transition' : 'all 0.2s ease-in-out','transform' : 'translateX(-100%)'});
	...
	/* ���� �Ѹ� ��� */
/*	function initMain(){
		if($(".banner_wrap").length < 1){ return;}
		$('.banner_wrap .banner').wrap('<div id="slider" class="swipe">');
		$(".banner_wrap").addClass("swipe-wrap");
